...
// so we need to find a proper way to discard *dirty* data from last request because of timeout, we need read up all dirty data either "after pconnect"(low performance, more reliable) or "after close"(high performance, less reliable, or we can register it as a shutdown function so that it is reliable, you can uncomment line "register_shutdown_function ... ", then visit or curl http://localhost/test_redis.php?ms=10000 to froce timeout, you will got file /tmp/tmp_shutdown_out)

error_reporting(E_ALL);
ini_set('display_errors', 'on');

$r = new Redis;
$r->pconnect('127.0.0.1'); //, 6380);
